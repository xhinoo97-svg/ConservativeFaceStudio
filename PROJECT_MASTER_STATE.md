...
- Current Track A blocker: DEC-009 same-canvas localized-damage attempt 1/3 **FAIL** with `1 failed,107 passed` in the targeted suite. The only failure proves the global same-canvas matcher must remain a canvas/background detector and must not require stable facial edges; the separate face-local gate is the identity authority.
...
| `hotfix/real-world-restoration-v1.1` | Track A | `2fcaeb1b...` | BLOCKED / ACTIVE | PR #2 OPEN/DRAFT; Release Quality #129 targeted FAIL 1/107; Windows/Female were still in progress at check | DEC-009 attempt2/3 |
...
- **PRODUCT_V1_1 — IMPLEMENTING/BLOCKED:** operational hotfix; current DEC-009 same-canvas hypothesis at attempt 2/3 boundary.
...
OBJ-002 Restore V1.1 gates — BLOCKED on DEC-009 attempt1; attempt2 planned.  
...
DEC-001 canonical ledger ACCEPTED. DEC-002 active Paper Quality branch ACCEPTED. DEC-003 <=80% CPU/process/system RAM + one heavy model ACCEPTED. DEC-004 evidence authority order ACCEPTED. DEC-005 mixed DamageMaskNet bank ACCEPTED. DEC-006 RefFace after DamageMaskNet ACCEPTED/BLOCKED. DEC-007 V3 consumed/V4 untouched ACCEPTED. DEC-008 ranking cluster != identity authority ACCEPTED/CLOSED after 3/3. **DEC-009 localized-damage same-canvas edge isolation — attempt1 FAIL, attempt2 ACCEPTED:** keep strict global Lab gate and local mismatch edge exclusion, but remove the requirement that a fixed proportion of global edges survive. Broad same-canvas is not identity authority; face-local bridge remains the identity gate. If fewer than 64 stable edges remain, edge consistency is simply non-authoritative and the already-passed strict photometric canvas rule governs broad match, matching the pre-existing contract.
...
**EXP-20260819-013 DEC-009 attempt2 plan:** preserve strict global Lab median/p90 and local mismatch dilation, remove only the `max(64,35% raw edges)` survival requirement. If stable edges >=64, validate them; otherwise do not turn broad canvas matching into an identity test. No SFace/face-local/provenance/model/holdout change.
...
Current `2fcaeb1b...`: DEC-009 attempt1 + stale fixture corrections. Release Quality #129 stopped at targeted suite with **1 failed,107 passed**; V3/V4 verification PASS without execution. The sole failure is architectural separation of broad same-canvas vs face-local identity. Windows/Female were still in progress at the status check and were not repeatedly polled.

Next exact action: DEC-009 attempt2/3 removes only the stable-edge survival threshold from the broad matcher. Then ledger update and one same-head status check. If targeted PASS, proceed full pytest/model/calibration/Windows/Female gates. V3/V4 remain unexecuted.
...
- **HIST-20260819-016:** Release Quality #129 on `2fcaeb1b...`: V3/V4 verify-only PASS; targeted `1 failed,107 passed`; sole failure broad same-canvas shared-background contract. Artifact `9364721505`, digest `37a71c20ca44af86d2a4e6f839246f0d34ba9287563f8082e6641f747978eb0c`. DEC-009 attempt1 consumed; attempt2 accepted before implementation.
