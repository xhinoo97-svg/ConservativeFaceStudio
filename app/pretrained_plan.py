...
PRETRAINED_BLOCK_PLAN: tuple[BlockModelChoice, ...] = (
    ...
    BlockModelChoice(
        BlockKind.DEBLUR,
        ("restormer_motion_deblur", "restormer_real_denoise"),
        "conservative OpenCV denoise/unsharp",
        "guarded-pretrained",
        "Restormer publishes separate official checkpoints for motion deblurring and real-image denoising; every neural result must pass the per-block identity guardrail.",
    ),
    BlockModelChoice(
        BlockKind.ENHANCE,
        ("mirnet_fivek_enhance",),
        "conservative CLAHE blend",
        "guarded-pretrained",
        "MIRNet publishes a pretrained FiveK enhancement model; use only when it improves validation without excessive colour or identity drift.",
    ),
    BlockModelChoice(
        BlockKind.LANDMARKS,
        ...
        "MediaPipe provides dense face landmarks; InsightFace provides detection plus 2D/3D alignment models in its model packs.",
    ),
    BlockModelChoice(
        BlockKind.ALIGN,
        ("mediapipe_face_landmarker", "insightface_identity"),
        ...
        "The model supplies geometry, while the actual alignment stays deterministic and auditable.",
    ),
    BlockModelChoice(
        ...
        "Face parsing supplies semantic support masks; reference consensus remains required before strict repair.",
    ),
    BlockModelChoice(
        ...
        "Strict mode uses same-identity observed pixels with provenance; DMDNet Full is an optional comparison/restoration backend.",
    ),
    BlockModelChoice(
        BlockKind.INPAINT,
        ("lama_big",),
    ...
    BlockModelChoice(
        ...
        "strict-memory-plus-optional-model",
        "DMDNet's specific-memory idea is useful, but strict fusion keeps exact reference-source provenance.",
    ),
    BlockModelChoice(
        ...
        "3DDFA_V2 supplies pretrained 3DMM pose/shape estimates; unseen texture must not be synthesized in strict mode.",
    ),
    BlockModelChoice(
        ...
        "ArcFace/InsightFace embeddings are substantially more meaningful than the histogram fallback for identity rollback.",
    ),
    BlockModelChoice(
        BlockKind.UPSCALE,
        ("realesrgan_x2plus",),
        "Lanczos",
        "generative-optional-only",
        "Real-ESRGAN is useful as an optional learned upscaler; Lanczos remains the strict default because it does not synthesize texture.",
    ),
    BlockModelChoice(
        BlockKind.EXPORT,
        (),
