...
def apply_exact_damage(face_bgr: np.ndarray, damage_class: str, seed: int) -> DamageSample:
    ...
    if damage_class == 'BLUR':
        ...
        for index in range(vertex_count):
            angle = (2 * math.pi * index / float(vertex_count)) + rng.uniform(-0.25, 0.25)
    ...
    return DamageSample(image=result, mask=label, damage_class=damage_class, seed=int(seed), source_id='')


def iter_balanced_samples(face: np.ndarray, source_id: str, *, samples_per_damage_class: int, base_seed: int) -> Iterable[DamageSample]:
    for class_index, name in enumerate(DAMAGE_CLASSES[1:], start=1):
        for repetition in range(int(samples_per_damage_class)):
            ...
            yield DamageSample(image=sample.image, mask=sample.mask, damage_class=name, seed=seed, source_id=source_id)
