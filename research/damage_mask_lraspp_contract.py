...
# Frozen before observing this architecture's result. These are DEVELOPMENT
# adequacy gates, not release or Target95 thresholds.
...
def verify_backbone_checkpoint(path: Path) -> str:
    return verify_file(
        path,
        expected_sha256=BACKBONE_SHA256,
        expected_bytes=BACKBONE_BYTES,
    )


def development_gate(
    *,
    damage_macro_f1: float,
    damage_macro_iou: float,
    per_damage_class_f1: list[float],
) -> dict[str, object]:
    checks = {
