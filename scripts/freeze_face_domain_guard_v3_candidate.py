...
"""Freeze the committed face-domain-guard-v3 candidate after calibration admission."""
...
def build(calibration_gate: Path, model_root: Path) -> dict[str, Any]:
    ...
    if gate.get("candidate_id") != CANDIDATE_ID or gate.get("accepted") is not True:
        raise RuntimeError("Calibration gate has not admitted face-domain-guard-v3")
    ...
    if not app_diff:
        raise RuntimeError("Candidate has no production app diff from verified pre-v3 HEAD")
