...
BENCHMARK_ROOT = REPOSITORY_ROOT / "benchmarks" / "face-smartphone-v2-final-holdout"
V1_ROOT = REPOSITORY_ROOT / "benchmarks" / "face-smartphone-v1"
CANVAS_SIZE = 384
BASE_SEED = 202608112000
BENCHMARK_ID = "cfs-face-smartphone-v2-final-holdout"
...
def _assert_independent(sources: list[dict[str, Any]]) -> str:
    old = _load(V1_ROOT / "sources.json")
    old_ids = {str(item["source_id"]) for item in old["sources"]}
    old_hashes = {str(item["clean_source_sha256"]) for item in old["sources"]}
    new_ids = {str(item["source_id"]) for item in sources}
    new_hashes = {str(item["clean_source_sha256"]) for item in sources}
    if new_ids & old_ids:
        raise RuntimeError(f"Final holdout source-id overlap with v1: {sorted(new_ids & old_ids)!r}")
    if new_hashes & old_hashes:
        raise RuntimeError("Final holdout clean-image checksum overlap with v1")
    frozen = _load(V1_ROOT / "freeze.json").get("sources_manifest_sha256")
    if not isinstance(frozen, str) or len(frozen) != 64:
        raise RuntimeError("v1 frozen source-manifest checksum missing")
    return frozen
...
def build_cases() -> dict[str, Any]:
    ...
    for category, count in CATEGORY_COUNTS.items():
        ...
        for category_index in range(count):
            ...
            cases.append({
                "case_id": f"cfsfs2-fin-{global_index + 1:03d}-{style}",
    ...
    return {
        ...
        "cases": cases,
        "generation_algorithm": "freeze_face_smartphone_v2_final_holdout.py:v1",
...
def build_freeze(cases_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "independent_from_consumed_v1_holdout": True,
        "mask_hashes_sha256": _sha(_canonical_json(masks)),
        "old_v1_sources_manifest_sha256": old_sources_sha,
...
def main() -> int:
    ...
    if args.verify:
        ...
        print("Face-smartphone v2 independent final holdout freeze: PASS")
        return 0
    BENCHMARK_ROOT.mkdir(parents=True, exist_ok=True)
    (BENCHMARK_ROOT / "cases.json").write_bytes(cases_bytes)
    (BENCHMARK_ROOT / "freeze.json").write_bytes(freeze_bytes)
    print("Face-smartphone v2 independent final holdout freeze generated: 40 cases")
    return 0
