...
"""Run face-domain-guard-v2 once on the independently frozen final holdout."""
...
def _verify_candidate_freeze(path: Path) -> dict:
    ...
    if payload.get("candidate_id") != CANDIDATE_ID:
        raise RuntimeError("Candidate freeze does not identify face-domain-guard-v2")
...
def run(output: Path, *, cache: Path, model_root: Path, candidate_freeze: Path) -> dict:
    ...
    # at the independently frozen v2 final holdout. No production pipeline code changes.
...
def main() -> int:
    ...
    parser.add_argument("--output", default="face-smartphone-v2-final-holdout-result")
    parser.add_argument("--cache", default=".benchmark-cache/face-smartphone-v2-final-holdout")
